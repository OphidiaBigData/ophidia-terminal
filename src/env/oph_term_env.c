...
int is_env_var_ok(const char *key) {
	...
	if (!strncmp(key,OPH_TERM_ENV_OPH_PREFIX,OPH_TERM_ENV_OPH_PREFIX_LEN)) return 0;
	int i;
	for (i = 0; i < (int) strlen(key); i++) {
		if (!isdigit(key[i])) return 1;
	}
	return 0;
}
...
int oph_term_get_user_vars(HASHTBL *hashtbl, char ***user_vars, int *user_vars_num) {
    ...
    *user_vars = NULL;
    ...
    for(n=0; n<hashtbl->size; n++) {
        ...
        while(node) {
            if (!is_env_var(node->key)) (*user_vars_num)++;
            node=node->next;
        }
    }
    ...
    for(n=0; n<hashtbl->size; n++) {
        ...
        while(node) {
            if (!is_env_var(node->key)) {
            	...
            	if (!((*user_vars)[i])) {
					for (j = 0; j < i; j++) free((*user_vars)[j]);
					free(*user_vars);
					*user_vars = NULL;
					*user_vars_num = 0;
					return OPH_TERM_MEMORY_ERROR;
...
int oph_term_env_update_xml(const char *repoURL,const char *userpwd,const char *local_dir) {
	...
	(print_json)?my_printf("Getting list of Ophidia operators XML files from \\\"%s\\\"... ",fixed_repoURL):printf("Getting list of Ophidia operators XML files from \"%s\"... ",fixed_repoURL);
	fflush(stdout);
	...
	while ((entry = readdir(dirp)) != NULL) {
		if (!strncmp(entry->d_name,"OPH_",4) && !strncmp(entry->d_name+strlen(entry->d_name)-4,".xml",4)) {
			...
			if (hashtbl_insert(*hashtbl,opnameptr,(void *)operator_args,operator_args_size)) {
				...
				*hashtbl = NULL;
				closedir(dirp);
				if (operator_args) free(operator_args);
				operator_args = NULL;
				return OPH_TERM_GENERIC_ERROR;
			}
			if (operator_args) free(operator_args);
...
int oph_term_view_filters_parsing(char *filters,int end, int **jobs, int *jobs_num) {
	...
	while (cursor1) {
		...
		if (left==0) {
			...
			if (!cursor3 || strcmp(cursor3,"end")) {
				(print_json)?my_fprintf(stderr, "Unable to parse markers: Invalid filter string.\\n"):fprintf(stderr, "\e[1;31mUnable to parse markers: Invalid filter string.\e[0m\n");
				if (job_flags) free(job_flags);
				return OPH_TERM_INVALID_PARAM_VALUE;
			}
		...
		if (is_range) {
			for (i = left; i <= right; i++) job_flags[i] = 1;
		} else {
			job_flags[left] = 1;
		}
	...
	//compute jobs_num
	*jobs_num = 0;
	for (i = 1; i <= end; i++) if (job_flags[i]) (*jobs_num)++;
...
int oph_term_get_session_size(char *session, char *user, char *passwd, char *host, char *port, int *return_value, int *size, int workflow_wrap, HASHTBL *hashtbl) {
	...
	if (oph_term_viewer_retrieve_session_size(value,size)) {
    ...
    for (i=0;i<(int)strlen(*expanded_string);i++) {
        if ((*expanded_string)[i]=='$' && (*expanded_string)[i+1]!='{') {
            for (j=i+1;j<(int)strlen(*expanded_string);j++) {
                if (!isalnum((*expanded_string)[j]) && (*expanded_string)[j]!='_') break;
            }
            if (j!=i+1) {
                c = (*expanded_string)[j];
                (*expanded_string)[j] = 0;
        ...
        for (i=0;i<(int)strlen(*expanded_string);i++) {
            if ((*expanded_string)[i]=='$' && (*expanded_string)[i+1]!='{') {
                ...
                if (m) {
                	(*expanded_string)[i] = 0;
                	memset(buf,0,OPH_TERM_WF_MAX_LEN);
                	snprintf(buf,OPH_TERM_WF_MAX_LEN,"%s%s%s",*expanded_string,(char *)hashtbl_get(hashtbl,buf2),((*expanded_string)[j]=='\0')?(*expanded_string)+j:(*expanded_string)+j+1);
                	memset(*expanded_string,0,OPH_TERM_WF_MAX_LEN);
                	snprintf(*expanded_string,OPH_TERM_WF_MAX_LEN,"%s",buf);
                	i+= strlen((char *)hashtbl_get(hashtbl,buf2)) -1;
                } else {
...
int oph_term_get_session_code(char *sessionid, char *code) {
    ...
    if (ptr) {
        ...
        } else return OPH_TERM_INVALID_PARAM_VALUE;
    } else return OPH_TERM_INVALID_PARAM_VALUE;
    return OPH_TERM_SUCCESS;
}
...
int oph_term_setalias(HASHTBL *hashtbl, const char *key, const char *value) {
	if (!is_alias_ok(key)) {
        (print_json)?my_fprintf(stderr, "An alias does not have to start with \\\"oph_\\\",\\\"/\\\" or \\\".\\\", it does not have to be a number or equal to an Oph_Term local command.\\n"):fprintf(stderr, "\e[1;31mAn alias does not have to start with \"oph_\",\"/\" or \".\", it does not have to be a number or equal to an Oph_Term local command.\e[0m\n");
        return OPH_TERM_INVALID_PARAM_VALUE;
	}
    if (hashtbl_insert(hashtbl,key,(void *)value,strlen(value)+1)) {
...
int oph_term_env_deploy(const char *auth_header,const char *infrastructure_url,const char *radl_filename, HASHTBL *hashtbl) {
	...
	if (http_code != 200 || ret == CURLE_ABORTED_BY_CALLBACK)
	{
		...
		if (mybuf.buffer) free(mybuf.buffer);
		return OPH_TERM_GENERIC_ERROR;
	}
	curl_easy_cleanup(hnd);
	...
	if (oph_term_setenv(hashtbl,OPH_TERM_ENV_OPH_INFRASTRUCTURE_URL,buffer)) {
		(print_json)?my_fprintf(stderr,"Could not set variable %s [CODE %d]\\n",OPH_TERM_ENV_OPH_INFRASTRUCTURE_URL,OPH_TERM_MEMORY_ERROR):fprintf(stderr,"\e[1;31mCould not set variable %s [CODE %d]\e[0m\n",OPH_TERM_ENV_OPH_INFRASTRUCTURE_URL,OPH_TERM_MEMORY_ERROR);
		if (mybuf.buffer) free(mybuf.buffer);
		return OPH_TERM_MEMORY_ERROR;
	}
...
int oph_term_env_get_server(const char *auth_header,const char *infrastructure_url, HASHTBL *hashtbl) {
	...
	if (oph_term_setenv(hashtbl,OPH_TERM_ENV_OPH_SERVER_HOST,ptr+strlen(OPH_IM_SERVER_INTERFACE))) {
		(print_json)?my_fprintf(stderr,"Could not set variable %s [CODE %d]\\n",OPH_TERM_ENV_OPH_SERVER_HOST,OPH_TERM_MEMORY_ERROR):fprintf(stderr,"\e[1;31mCould not set variable %s [CODE %d]\e[0m\n",OPH_TERM_ENV_OPH_SERVER_HOST,OPH_TERM_MEMORY_ERROR);
		if (mybuf.buffer) free(mybuf.buffer);
		return OPH_TERM_MEMORY_ERROR;
	}
	...
	if (oph_term_setenv(hashtbl,OPH_TERM_ENV_OPH_SERVER_PORT,OPH_IM_SERVER_DEFAULT_PORT)) {
		(print_json)?my_fprintf(stderr,"Could not set variable %s [CODE %d]\\n",OPH_TERM_ENV_OPH_SERVER_PORT,OPH_TERM_MEMORY_ERROR):fprintf(stderr,"\e[1;31mCould not set variable %s [CODE %d]\e[0m\n",OPH_TERM_ENV_OPH_SERVER_PORT,OPH_TERM_MEMORY_ERROR);
		if (mybuf.buffer) free(mybuf.buffer);
		return OPH_TERM_MEMORY_ERROR;
	}
