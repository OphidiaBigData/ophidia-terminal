...
int base64decode (const char *in, size_t inLen, char *out, size_t *outLen) { 
    ...
    if (iter == 3) {
        if ((len += 2) > *outLen) return 3; /* buffer overflow */
        (*out) = (buf >> 10) &255;
	out++;
        (*out) = (buf >> 2) &255;
	out++;
        ...
        if (++len > *outLen) return 4; /* buffer overflow */
        (*out) = (buf >> 4) &255;
	out++;
    }

    if (++len > *outLen) return 5; /* buffer overflow */
    (*out) = 0;
    out++;
...
size_t _write_callback(char *ptr, size_t size, size_t nmemb, void *userdata)
{
	...
	else if (store_result_global) // Async mode
	{
		...
		if(xpathObj->nodesetval) mem->error = OPH_SERVER_OK;
		xmlXPathFreeObject(xpathObj);
		...
		if (node)
		{
			...
			if (attr && attr->children) mem->xml = strdup((const char *)attr->children->content);
		}
		xmlXPathFreeObject(xpathObj);
	}
	else // Sync mode
...
int wps_call_oph__ophExecuteMain(char* server_global, char* query_global, char* username, char* password, int store_result)
{
	...
	snprintf(wpsRequest,WORKFLOW_MAX_LEN,OPH_WPS_XML_REQUEST,query_global,username,password,store_result?"storeExecuteResponse=\"true\" status=\"true\"":"storeExecuteResponse=\"false\"");
...
void oph_execute(char* query, char **newsession, int *return_value, char **out_response, char **out_response_for_viewer, int workflow_wrap, char *username, char *password, HASHTBL *hashtbl, char *cmd_line)
{
		...
		if (tmp_query) {
			...
			while (i < len) {
				if (tmp_query[i] == '=') {
					...
					skip=0;
					for (j = i+1; j < len; j++) {
						if (tmp_query[j] == '[') skip=1;
						else if (tmp_query[j] == ']') skip=0;
		...
		if (query_start) {
			...
			if (hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SESSION_ID)) {
				if (!strstr(query,"\"sessionid\"")) {
					n += snprintf(fixed_query+n,WORKFLOW_MAX_LEN-n,"%s%s%s",WRAPPING_WORKFLOW2,(char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SESSION_ID),WRAPPING_WORKFLOW2_1);
				}
			}
			if (!(tmp = strstr(query,"\"exec_mode\""))) {
				...
				if ((tmp = hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_EXEC_MODE)) && strstr(tmp,"async")) store_result_global=1;
			}
			else if ((tmp = strstr(tmp,"\"async\""))) { *tmp = ' '; *(tmp+1)='\"'; store_result_global=1; };
			if (hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_NCORES)) {
				if (!strstr(query,"\"ncores\"")) {
					n += snprintf(fixed_query+n,WORKFLOW_MAX_LEN-n,"%s%s%s",WRAPPING_WORKFLOW4c,(char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_NCORES),WRAPPING_WORKFLOW4c_1);
				}
			}
			...
			if (hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_DATACUBE)) {
				if (!strstr(query,"\"cube\"")) {
					n += snprintf(fixed_query+n,WORKFLOW_MAX_LEN-n,"%s%s%s",WRAPPING_WORKFLOW4e,(char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_DATACUBE),WRAPPING_WORKFLOW4e_1);
				}
			}
			if (cmd_line) {
	...
	if (!response_global.error && response_global.response)
	{
		...
		if (outLen) response_global.response = strdup(result);
		else response_global.response = NULL;
	}
				...
				if (out_response) {
					if (!response_global.response) {
						...
						*out_response = strdup((const char *) response_global.response);
						...
						if (strstr(response_global.response,"\"title\": \"ERROR\"") || (strstr(response_global.response,"\"title\": \"Workflow Status\"") && strstr(response_global.response,"\"message\": \"OPH_STATUS_ERROR\"")))
							*return_value = OPH_TERM_GENERIC_ERROR;
					}
				} else {
					if (print_json) *oph_term_jobid=0;
					if (!response_global.response) {
						...
						if (!print_json) printf("\e[1;34m[Response]:\e[0m\n");
						if (strstr(response_global.response,"\"title\": \"ERROR\"") || (strstr(response_global.response,"\"title\": \"Workflow Status\"") && strstr(response_global.response,"\"message\": \"OPH_STATUS_ERROR\"")))
							*return_value = OPH_TERM_GENERIC_ERROR;
					}
...
int oph_term_client(char *cmd_line,char *command,char **newsession,char *user,char *password,char *host,char *port, int *return_value, char **out_response, char **out_response_for_viewer, int workflow_wrap, HASHTBL *hashtbl)
{
    ...
    if (command) snprintf(query_global,WORKFLOW_MAX_LEN,"%s",command);

    char *query = query_global;

    if (!strcasecmp(query,OPH_DEFAULT_QUERY)) return 1;
...
void sigpipe_handle(int x) { UNUSED(x)}
