...
void oph_execute(struct soap *soap, xsd__string query, char* wps, char **newsession, int *return_value, char **out_response, char **out_response_for_viewer, int workflow_wrap, char *username, HASHTBL *hashtbl, char *cmd_line)
{
		...
		if (query_start) {
			...
			if (hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_EXEC_MODE)) {
				if (!strstr(query,"\"exec_mode\"")) {
					n += snprintf(fixed_query+n,WORKFLOW_MAX_LEN-n,"%s%s%s",WRAPPING_WORKFLOW3,(char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_EXEC_MODE),WRAPPING_WORKFLOW3_1);
				}
			}
