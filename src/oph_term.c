...
void print_startup_usage(char *arg0,FILE *stream) {
    ...
    (print_json)?my_fprintf(stream,"   When the GSI interface is used, the variables OPH_USER and OPH_PASSWD are ignored and the user certificate is used for authentication.\\n"):fprintf(stream,"   When the GSI interface is used, the variables OPH_USER and OPH_PASSWD are ignored and the user certificate is used for authentication.\n");
    (print_json)?my_fprintf(stream,"   User-defined variables can be specified using OPH_TERM_USERVAR_X (with X any string)\\n"):fprintf(stream,"   User-defined variables can be specified using OPH_TERM_USERVAR_X (with X any string)\n");
    (print_json)?my_fprintf(stream,"   and the pattern <OPH_TERM_USERVAR_X='uservarname=uservarvalue'> (note the single quotes).\\n"):fprintf(stream,"   and the pattern <OPH_TERM_USERVAR_X='uservarname=uservarvalue'> (note the single quotes).\n");
    (print_json)?my_fprintf(stream,"   Oph_Term will search for all of these special variables in your environment.\\n"):fprintf(stream,"   Oph_Term will search for all of these special variables in your environment.\n");
    (print_json)?my_fprintf(stream,"   The result will be equal to calling <setenv uservarname=uservarvalue> from within Oph_Term.\\n"):fprintf(stream,"   The result will be equal to calling <setenv uservarname=uservarvalue> from within Oph_Term.\n");
    (print_json)?my_fprintf(stream,"   User-defined aliases can be specified using OPH_TERM_ALIAS_X (with X any string)\\n"):fprintf(stream,"   User-defined aliases can be specified using OPH_TERM_ALIAS_X (with X any string)\n");
    (print_json)?my_fprintf(stream,"   and the pattern <OPH_TERM_ALIAS_X='aliasname=aliasvalue'> (note the single quotes).\\n"):fprintf(stream,"   and the pattern <OPH_TERM_ALIAS_X='aliasname=aliasvalue'> (note the single quotes).\n");
    (print_json)?my_fprintf(stream,"   Oph_Term will search for all of these special variables in your environment.\\n"):fprintf(stream,"   Oph_Term will search for all of these special variables in your environment.\n");
    (print_json)?my_fprintf(stream,"   The result will be equal to calling <setalias aliasname=aliasvalue> from within Oph_Term.\\n"):fprintf(stream,"   The result will be equal to calling <setalias aliasname=aliasvalue> from within Oph_Term.\n");
    ...
    (print_json)?my_fprintf(stream,"   interact with the server in order to resume the last remote session the user connected to.\\n"):fprintf(stream,"   interact with the server in order to resume the last remote session the user connected to.\n");
    (print_json)?my_fprintf(stream,"   When the GSI interface is used, the variables OPH_USER and OPH_PASSWD are ignored and the user certificate is used for authentication.\\n"):fprintf(stream,"   When the GSI interface is used, the variables OPH_USER and OPH_PASSWD are ignored and the user certificate is used for authentication.\n");
    (print_json)?my_fprintf(stream,"   OPH_SESSION_ID,OPH_EXEC_MODE,OPH_NCORES,OPH_CWD and OPH_DATACUBE will be inserted in\\n"):fprintf(stream,"   OPH_SESSION_ID,OPH_EXEC_MODE,OPH_NCORES,OPH_CWD and OPH_DATACUBE will be inserted in\n");
    (print_json)?my_fprintf(stream,"   the JSON request (original file will remain untouched) if not already present.\\n"):fprintf(stream,"   the JSON request (original file will remain untouched) if not already present.\n");
    (print_json)?my_fprintf(stream,"   \\\"file\\\" must be the name of a valid JSON file according to the Ophidia Workflow JSON Schema.\\n"):fprintf(stream,"   \"file\" must be the name of a valid JSON file according to the Ophidia Workflow JSON Schema.\n");
    (print_json)?my_fprintf(stream,"   With -a it is possible to substitute placeholders like $1,$2 etc in the JSON request\\n"):fprintf(stream,"   With -a it is possible to substitute placeholders like $1,$2 etc in the JSON request\n");
...
int startup_opt_setup(int argc, char *argv[], char *envp[], HASHTBL *hashtbl, char **exec_statement) {
    ...
    if (oph_term_setenv(hashtbl,OPH_TERM_ENV_OPH_EXEC_MODE,"sync")) {
        (print_json)?my_fprintf(stderr,"Could not set variable %s [CODE %d]\\n",OPH_TERM_ENV_OPH_EXEC_MODE,OPH_TERM_MEMORY_ERROR):fprintf(stderr,"\e[1;31mCould not set variable %s [CODE %d]\e[0m\n",OPH_TERM_ENV_OPH_EXEC_MODE,OPH_TERM_MEMORY_ERROR);
        return OPH_TERM_MEMORY_ERROR;
    }
    //preset OPH_GRAPH_LAYOUT
    if (oph_term_setenv(hashtbl,OPH_TERM_ENV_OPH_GRAPH_LAYOUT,"vertical_compact")) {
        (print_json)?my_fprintf(stderr,"Could not set variable %s [CODE %d]\\n",OPH_TERM_ENV_OPH_GRAPH_LAYOUT,OPH_TERM_MEMORY_ERROR):fprintf(stderr,"\e[1;31mCould not set variable %s [CODE %d]\e[0m\n",OPH_TERM_ENV_OPH_GRAPH_LAYOUT,OPH_TERM_MEMORY_ERROR);
        return OPH_TERM_MEMORY_ERROR;
    }
    //preset OPH_RESPONSE_BUFFER
    if (oph_term_setenv(hashtbl,OPH_TERM_ENV_OPH_RESPONSE_BUFFER,"1024")) {
        (print_json)?my_fprintf(stderr,"Could not set variable %s [CODE %d]\\n",OPH_TERM_ENV_OPH_RESPONSE_BUFFER,OPH_TERM_MEMORY_ERROR):fprintf(stderr,"\e[1;31mCould not set variable %s [CODE %d]\e[0m\n",OPH_TERM_ENV_OPH_RESPONSE_BUFFER,OPH_TERM_MEMORY_ERROR);
        return OPH_TERM_MEMORY_ERROR;
    }
    //preset OPH_WORKFLOW_AUTOVIEW
    if (oph_term_setenv(hashtbl,OPH_TERM_ENV_OPH_WORKFLOW_AUTOVIEW,"off")) {
        (print_json)?my_fprintf(stderr,"Could not set variable %s [CODE %d]\\n",OPH_TERM_ENV_OPH_WORKFLOW_AUTOVIEW,OPH_TERM_MEMORY_ERROR):fprintf(stderr,"\e[1;31mCould not set variable %s [CODE %d]\e[0m\n",OPH_TERM_ENV_OPH_WORKFLOW_AUTOVIEW,OPH_TERM_MEMORY_ERROR);
        return OPH_TERM_MEMORY_ERROR;
    }
    ...
    for (z = 0; z < pre_defined_aliases_num; z++) {
        if (oph_term_setalias(alias_ptr,pre_defined_aliases_keys[z],pre_defined_aliases_values[z])) {
            (print_json)?my_fprintf(stderr,"Could not set alias %s [CODE %d]\\n",pre_defined_aliases_keys[z],OPH_TERM_MEMORY_ERROR):fprintf(stderr,"\e[1;31mCould not set alias %s [CODE %d]\e[0m\n",pre_defined_aliases_keys[z],OPH_TERM_MEMORY_ERROR);
            return OPH_TERM_MEMORY_ERROR;
        }
	}
    ...
    for (i = 0; envp[i]; i++) {
		if (!strncmp(envp[i],OPH_TERM_USERVAR_PATTERN,17)) {
			...
			size_t q,z;
        	for (q = 0; q < strlen(buf); q++) if (buf[q]=='=') break;
        	if (q==strlen(buf)) {
                (print_json)?my_fprintf(stderr,"Invalid format for %s [CODE %d]\\n",buf,OPH_TERM_INVALID_PARAM_VALUE):fprintf(stderr,"\e[1;31mInvalid format for %s [CODE %d]\e[0m\n",buf,OPH_TERM_INVALID_PARAM_VALUE);
                return OPH_TERM_INVALID_PARAM_VALUE;
			}
        	for (z = q+1; z < strlen(buf); z++) if (buf[z]=='=') break;
        	if (z==strlen(buf)) {
                (print_json)?my_fprintf(stderr,"Invalid format for %s [CODE %d]\\n",buf,OPH_TERM_INVALID_PARAM_VALUE):fprintf(stderr,"\e[1;31mInvalid format for %s [CODE %d]\e[0m\n",buf,OPH_TERM_INVALID_PARAM_VALUE);
                return OPH_TERM_INVALID_PARAM_VALUE;
			}
            ...
            if (oph_term_setenv(hashtbl,buf+q+1,buf+z+1)) {
                (print_json)?my_fprintf(stderr,"Could not set variable %s [CODE %d]\\n",buf+q+1,OPH_TERM_MEMORY_ERROR):fprintf(stderr,"\e[1;31mCould not set variable %s [CODE %d]\e[0m\n",buf+q+1,OPH_TERM_MEMORY_ERROR);
                return OPH_TERM_MEMORY_ERROR;
            }
		}
	}
    ...
    for (i = 0; envp[i]; i++) {
		if (!strncmp(envp[i],OPH_TERM_ALIAS_PATTERN,15)) {
            ...
            if (oph_term_setalias(alias_ptr,buf+q+1,buf+z+1)) {
                (print_json)?my_fprintf(stderr,"Could not set alias %s [CODE %d]\\n",buf+q+1,OPH_TERM_MEMORY_ERROR):fprintf(stderr,"\e[1;31mCould not set alias %s [CODE %d]\e[0m\n",buf+q+1,OPH_TERM_MEMORY_ERROR);
                return OPH_TERM_MEMORY_ERROR;
            }
		}
	}
...
char *oph_term_base_generator(const char *text, int state) {
  ...
  if (user_vars_list_num) {
	  while (list_index>=(cmds_list_size+def_vars_list_size) && list_index<(cmds_list_size+def_vars_list_size+user_vars_list_num))
	  {
		  ...
		  if (strncmp (name, text, len) == 0) {
			  ...
			  for (i = 0; i < user_vars_list_num; i++) free(user_vars_list[i]);
			  free(user_vars_list);
			  return match;
		  }
	  }
  ...
  if (aliases_list_num) {
	  while (list_index>=(cmds_list_size+def_vars_list_size+user_vars_list_num) && list_index<(cmds_list_size+def_vars_list_size+user_vars_list_num+aliases_list_num))
	  {
		  ...
		  if (strncmp (name, text, len) == 0) {
			  char *match = strdup(name);
			  for (i = 0; i < aliases_list_num; i++) free(aliases_list[i]);
			  free(aliases_list);
			  return match;
		  }
	  }
...
char *oph_term_arguments_generator(const char *text, int state) {
  ...
  {
	if (rl_line_buffer[jj]=='[') bracket++;
	else if ((rl_line_buffer[jj]==']') && (bracket>0)) bracket--;
  }
...
char *oph_term_arg_values_generator(const char *text, int state) {
  ...
  if (args[argindex].values[0]!='\0') {
	  ...
	  for (i = 0; i < (int)strlen(args[argindex].values); i++) {
		  if (args[argindex].values[i]=='|') values_num++;
	  }
...
char **oph_term_completion(char *text, int start, int end) {
	...
	} else if (rl_line_buffer[(start-1 < 0)?start:start-1]=='$' || (rl_line_buffer[(start-2 < 0)?start:start-2]=='$' && rl_line_buffer[(start-1 < 0)?start:start-1]=='{')) {
		// completion over env vars (def+user)
		if (rl_line_buffer[(start-1 < 0)?start:start-1]=='$') rl_completion_suppress_append = 1;
										...
										if (!strcmp(matches[i],args[k].name)) {
											if (args[k].mandatory == 'M') {
												...
												if (args[k].values[0]!='\0' && args[k].default_val[0]!='\0') {
													...
													if (tmpmatch) {
														memset(tmpmatch,0,200);
														snprintf(tmpmatch,200,"%s [%s (%s)]",args[k].name,args[k].values,args[k].default_val);
														free(matches[i]);
														matches[i] = tmpmatch;
													}
...
void print_oph_term_output_json(HASHTBL *hashtbl)
{
	...
	else printf(OUTPUT_JSON_STRING,oph_term_request_exp,oph_term_jobid_exp,oph_term_response?oph_term_response:"{}",oph_term_output_exp,oph_term_error_exp);
	...
	if (oph_term_response) { free(oph_term_response); oph_term_response = NULL; }
...
int main(int argc, char **argv, char **envp) {
    ...
    for (i = 0; i < argc; i++) {
        if (!strncmp(argv[i],"--password=",11) || (!strncmp(argv[i],"-p",2) && argv[i][2] != 0)) continue;
        if (!strncmp(argv[i],"--password",10) || (!strncmp(argv[i],"-p",2) && argv[i][2] == 0)) { i++;continue; }
    	n += snprintf(command_line+n,OPH_TERM_MAX_LEN-n,"%s ",argv[i]);
	}

    if (!print_json) printf("\e[2m");

	// Init OPH_SESSION_ID if empty
    if (!hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SESSION_ID)) {
#ifndef INTERFACE_TYPE_IS_GSI
    	if (hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SERVER_HOST) && hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SERVER_PORT) && hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_USER) && hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_PASSWD)) {
#else
    	if (hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SERVER_HOST) && hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SERVER_PORT)) {
    		...
    		if (oph_term_env_oph_get_config(OPH_TERM_SESSION_ID_KEY,hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SERVER_HOST),hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SERVER_PORT),&oph_term_return,&last_sessionid,hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_USER),hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_PASSWD),1,hashtbl)) {
    		...
    		if (last_sessionid && strlen(last_sessionid)) {
        	    if (oph_term_setenv(hashtbl,OPH_TERM_ENV_OPH_SESSION_ID,last_sessionid)) {
        			oph_term_env_clear(hashtbl);
        			oph_term_alias_clear(aliases);
        			(print_json)?my_fprintf(stderr,"Could not set variable %s [CODE %d]\\n",OPH_TERM_ENV_OPH_SESSION_ID,OPH_TERM_MEMORY_ERROR):fprintf(stderr,"\e[1;31mCould not set variable %s [CODE %d]\e[0m\n",OPH_TERM_ENV_OPH_SESSION_ID,OPH_TERM_MEMORY_ERROR);
        			if (!print_json) printf("\e[0m");
        			if (exec_statement) {
        				free(exec_statement);
        				exec_statement = NULL;
        			}
        	    ...
        	    if (!hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_CWD)) {
            		char *last_cwd = NULL;
            		if (oph_term_env_oph_get_config(OPH_TERM_OPH_CWD_KEY,hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SERVER_HOST),hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SERVER_PORT),&oph_term_return,&last_cwd,hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_USER),hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_PASSWD),1,hashtbl)) {
            		...
            		if (last_cwd && strlen(last_cwd)) {
                	    if (oph_term_setenv(hashtbl,OPH_TERM_ENV_OPH_CWD,last_cwd)) {
                			oph_term_env_clear(hashtbl);
                			oph_term_alias_clear(aliases);
                			(print_json)?my_fprintf(stderr,"Could not set variable %s [CODE %d]\\n",OPH_TERM_ENV_OPH_CWD,OPH_TERM_MEMORY_ERROR):fprintf(stderr,"\e[1;31mCould not set variable %s [CODE %d]\e[0m\n",OPH_TERM_ENV_OPH_CWD,OPH_TERM_MEMORY_ERROR);
                			if (!print_json) printf("\e[0m");
                			if (exec_statement) {
                				free(exec_statement);
                				exec_statement = NULL;
        	    ...
        	    if (!hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_DATACUBE)) {
        	    	char *last_cube = NULL;
        	    	if (oph_term_env_oph_get_config(OPH_TERM_OPH_DATACUBE_KEY,hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SERVER_HOST),hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SERVER_PORT),&oph_term_return,&last_cube,hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_USER),hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_PASSWD),1,hashtbl)) {
        	    	...
        	    	if (last_cube && strlen(last_cube)) {
        	    		if (oph_term_setenv(hashtbl,OPH_TERM_ENV_OPH_DATACUBE,last_cube)) {
        	    			oph_term_env_clear(hashtbl);
        	    			oph_term_alias_clear(aliases);
        	    			(print_json)?my_fprintf(stderr,"Could not set variable %s [CODE %d]\\n",OPH_TERM_ENV_OPH_DATACUBE,OPH_TERM_MEMORY_ERROR):fprintf(stderr,"\e[1;31mCould not set variable %s [CODE %d]\e[0m\n",OPH_TERM_ENV_OPH_DATACUBE,OPH_TERM_MEMORY_ERROR);
        	    			if (!print_json) printf("\e[0m");
        	    			if (exec_statement) {
        	    				free(exec_statement);
        	    				exec_statement = NULL;
    ...
    if (hashtbl_get(hashtbl,OPH_TERM_OPT_W_FILENAME)) {
        ...
        if (!hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_TERM_VIEWER) ||
        	...
        	(print_json)?my_fprintf(stderr,"OPH_TERM_VIEWER not set or incorrect [CODE %d]\\n",OPH_TERM_INVALID_PARAM_VALUE):fprintf(stderr,"\e[1;31mOPH_TERM_VIEWER not set or incorrect [CODE %d]\e[0m\n",OPH_TERM_INVALID_PARAM_VALUE);
            oph_term_env_clear(hashtbl);
            oph_term_alias_clear(aliases);
            if (print_json) print_oph_term_output_json(hashtbl);
            return OPH_TERM_INVALID_PARAM_VALUE;
	...
	int error;
        ...
        oph_term_client(command_line,submission_workflow,&session,(char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_USER),(char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_PASSWD),(char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SERVER_HOST),(char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SERVER_PORT),&oph_term_return,NULL,&response_for_viewer,0,hashtbl);
        if (submission_workflow) {
        	free(submission_workflow);
        	submission_workflow = NULL;
        }
		...
		if (!print_debug_data && response_for_viewer && ((int)strlen(response_for_viewer) > 1024*strtol((char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_RESPONSE_BUFFER),NULL,10)))
		{
			...
			(print_json)?my_fprintf(stderr,"Response exceeds the maximum size %s kB.\\n",(char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_RESPONSE_BUFFER)):fprintf(stderr,"\e[1;31mResponse exceeds the maximum size %s kB.\e[0m\n",(char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_RESPONSE_BUFFER));
		}
        ...
        if (hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_WORKFLOW_AUTOVIEW) && last_jobid>0) {
        	...
        	if (!autoview) {
        		(print_json)?my_fprintf(stderr,"Could not parse OPH_WORKFLOW_AUTOVIEW [CODE %d]\\n",OPH_TERM_MEMORY_ERROR):fprintf(stderr,"\e[1;31mCould not parse OPH_WORKFLOW_AUTOVIEW [CODE %d]\e[0m\n",OPH_TERM_MEMORY_ERROR);
                oph_term_env_clear(hashtbl);
                oph_term_alias_clear(aliases);
                if (print_json) print_oph_term_output_json(hashtbl);
                return OPH_TERM_GENERIC_ERROR;
        	...
        	if (!token) {
        		free(autoview);
        		(print_json)?my_fprintf(stderr,"Invalid OPH_WORKFLOW_AUTOVIEW value [CODE %d]\\n",OPH_TERM_INVALID_PARAM_VALUE):fprintf(stderr,"\e[1;31mInvalid OPH_WORKFLOW_AUTOVIEW value [CODE %d]\e[0m\n",OPH_TERM_INVALID_PARAM_VALUE);
                oph_term_env_clear(hashtbl);
                oph_term_alias_clear(aliases);
                if (print_json) print_oph_term_output_json(hashtbl);
                return OPH_TERM_GENERIC_ERROR;
        	...
        	if (!strcmp(token,"on")) {
        		...
        		if (!exec_statement) {
        			free(autoview);
        			(print_json)?my_fprintf(stderr,"Memory error calling view [CODE %d]\\n",OPH_TERM_MEMORY_ERROR):fprintf(stderr,"\e[1;31mMemory error calling view [CODE %d]\e[0m\n",OPH_TERM_MEMORY_ERROR);
                    oph_term_env_clear(hashtbl);
                    oph_term_alias_clear(aliases);
                    if (print_json) print_oph_term_output_json(hashtbl);
                    return OPH_TERM_GENERIC_ERROR;
    ...
    if (!exec_statement) {
    	...
    	if (!xml_path) {
    		...
    		oph_term_alias_clear(aliases);
    		free(hist_path);
    		(print_json)?my_fprintf(stderr,"Unable to set XML folder path [CODE %d]\\n",OPH_TERM_MEMORY_ERROR):fprintf(stderr,"\e[1;31mUnable to set XML folder path [CODE %d]\e[0m\n",OPH_TERM_MEMORY_ERROR);
    		if (exec_statement) {
    			free(exec_statement);
    			exec_statement = NULL;
    		}
    	...
    	if (mkdir(xml_path,0777)) {
    		if (errno != EEXIST) {
    			...
    			free(hist_path);
    			free(xml_path);
    			(print_json)?my_fprintf(stderr,"Unable to create XML folder [CODE %d]\\n",OPH_TERM_GENERIC_ERROR):fprintf(stderr,"\e[1;31mUnable to create XML folder [CODE %d]\e[0m\n",OPH_TERM_GENERIC_ERROR);
    			if (exec_statement) {
    				free(exec_statement);
    				exec_statement = NULL;
    			}
    	...
    	if (hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SERVER_HOST) && hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SERVER_PORT)) {
    		...
    		char *tmpurl = NULL;
    		if (oph_term_env_oph_get_config(OPH_TERM_XML_URL_KEY,hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SERVER_HOST),hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SERVER_PORT),&oph_term_return,&tmpurl,hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_USER),hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_PASSWD),1,hashtbl)) {
    		...
    		if (tmpurl) {
            	...
            	if (mkdir(xml_path_extended,0777)) {
            		if (errno != EEXIST) {
            			...
            			free(xml_path);
            			free(tmpurl);
            			(print_json)?my_fprintf(stderr,"Unable to create XML folder %s [CODE %d]\\n",xml_path_extended,OPH_TERM_GENERIC_ERROR):fprintf(stderr,"\e[1;31mUnable to create XML folder %s [CODE %d]\e[0m\n",xml_path_extended,OPH_TERM_GENERIC_ERROR);
            			if (exec_statement) {
            				free(exec_statement);
            				exec_statement = NULL;
            			}
        ...
        if (line && *line)
	{
        ...
        else if (!strcmp(cursor,OPH_TERM_CMD_VERSION))
        { // VERSION
        	alias_substitutions = 0;
            print_version();
            if (print_json) print_oph_term_output_json(hashtbl);
        ...
        else if (!strcmp(cursor,OPH_TERM_CMD_WARRANTY))
        { // WARRANTY
        	alias_substitutions = 0;
            print_version();
            (print_json)?my_printf("%s",OPH_TERM_WARRANTY2):printf("%s",OPH_TERM_WARRANTY);
        ...
        else if (!strcmp(cursor,OPH_TERM_CMD_CONDITIONS))
        { // CONDITIONS
        	alias_substitutions = 0;
            print_version();
            (print_json)?my_printf("%s",OPH_TERM_CONDITIONS2):printf("%s",OPH_TERM_CONDITIONS);
        ...
        else if (!strcmp(cursor,OPH_TERM_CMD_HISTORY))
        { // HISTORY
        	alias_substitutions = 0;
        ...
        else if (!strncmp(cursor,OPH_TERM_REMOTE_CMD_PREFIX,OPH_TERM_REMOTE_CMD_PREFIX_LEN))
        { // REMOTE COMMAND OPH_*
        	alias_substitutions = 0;
            ...
            if (!hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_TERM_IMGS) ||
            		(strcmp((char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_TERM_IMGS),"no_op") &&
            		 ...
            		 strcmp((char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_TERM_IMGS),"open"))) {
                (print_json)?my_fprintf(stderr,"OPH_TERM_IMGS not set or incorrect [CODE %d]\\n",OPH_TERM_INVALID_PARAM_VALUE):fprintf(stderr,"\e[1;31mOPH_TERM_IMGS not set or incorrect [CODE %d]\e[0m\n",OPH_TERM_INVALID_PARAM_VALUE);
                if (print_json) print_oph_term_output_json(hashtbl);
                if (exec_one_statement) {
            		oph_term_return = OPH_TERM_INVALID_PARAM_VALUE;
            		break;
            ...
            if (!current_operator) {
                (print_json)?my_fprintf(stderr,"Unable to duplicate operator name [CODE %d]\\n",OPH_TERM_MEMORY_ERROR):fprintf(stderr,"\e[1;31mUnable to duplicate operator name [CODE %d]\e[0m\n",OPH_TERM_MEMORY_ERROR);
                if (print_json) print_oph_term_output_json(hashtbl);
                if (exec_one_statement) {
            		oph_term_return = OPH_TERM_MEMORY_ERROR;
            		break;
            ...
            if (!cursor) {
            	...
            	if (!hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_CWD)) {
            		if (oph_term_setenv(hashtbl,OPH_TERM_ENV_OPH_CWD,"/")) {
            			(print_json)?my_fprintf(stderr,"Could not set cwd [CODE %d]\\n",OPH_TERM_MEMORY_ERROR):fprintf(stderr,"\e[1;31mCould not set cwd [CODE %d]\e[0m\n",OPH_TERM_MEMORY_ERROR);
            			if (print_json) print_oph_term_output_json(hashtbl);
            			if (exec_one_statement) {
            	    		oph_term_return = OPH_TERM_MEMORY_ERROR;
            	    		break;
            	...
            	char *response_for_viewer = NULL;
            	oph_term_client(command_line,submission_string,&newsession,(char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_USER),(char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_PASSWD),(char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SERVER_HOST),(char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SERVER_PORT),&oph_term_return,NULL,&response_for_viewer,1,hashtbl);
		if (!print_debug_data && response_for_viewer && ((int)strlen(response_for_viewer) > 1024*strtol((char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_RESPONSE_BUFFER),NULL,10)))
		{
			if (response_for_viewer) free(response_for_viewer);
			response_for_viewer = NULL;
			(print_json)?my_fprintf(stderr,"Response exceeds the maximum size %s kB.\\n",(char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_RESPONSE_BUFFER)):fprintf(stderr,"\e[1;31mResponse exceeds the maximum size %s kB.\e[0m\n",(char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_RESPONSE_BUFFER));
            	...
            	if (newsession) {
            		if (strlen(newsession)==0) {
            		...
            		} else if ((hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SESSION_ID) && strcmp((char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SESSION_ID),newsession)) || !hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SESSION_ID)) {
            			if (oph_term_setenv(hashtbl,OPH_TERM_ENV_OPH_SESSION_ID,newsession)) {
            				(print_json)?my_fprintf(stderr,"Could not set OPH_SESSION_ID [CODE %d]\\n",OPH_TERM_MEMORY_ERROR):fprintf(stderr,"\e[1;31mCould not set OPH_SESSION_ID [CODE %d]\e[0m\n",OPH_TERM_MEMORY_ERROR);
            				free(newsession);
            				newsession = NULL;
            				if (newdatacube) {
            					free(newdatacube);
            			...
            			if (oph_term_setenv(hashtbl,OPH_TERM_ENV_OPH_CWD,"/")) {
            				(print_json)?my_fprintf(stderr,"Could not set OPH_CWD [CODE %d]\\n",OPH_TERM_MEMORY_ERROR):fprintf(stderr,"\e[1;31mCould not set OPH_CWD [CODE %d]\e[0m\n",OPH_TERM_MEMORY_ERROR);
            				free(newsession);
            				newsession = NULL;
            				if (newdatacube) {
            					free(newdatacube);
            	...
            	if (newdatacube) {
                	if (massive_flag) {
                		...
                		if (oph_term_setenv(hashtbl,OPH_TERM_ENV_OPH_DATACUBE,newdatacube)) {
                			(print_json)?my_fprintf(stderr,"Could not set OPH_DATACUBE [CODE %d]\\n",OPH_TERM_MEMORY_ERROR):fprintf(stderr,"\e[1;31mCould not set OPH_DATACUBE [CODE %d]\e[0m\n",OPH_TERM_MEMORY_ERROR);
                			free(newdatacube);
                			newdatacube = NULL;
                			if (newcwd) {
                				free(newcwd);
                ...
                if (!strstr(cursor,";exec_mode=") && strncmp(cursor,"exec_mode=",10)) {
                    if (hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_EXEC_MODE)) {
                        if (!flag) {
                            ...
                            if (!cursorcopy2) {
                                (print_json)?my_fprintf(stderr,"Could not use OPH_EXEC_MODE [CODE %d]\\n",OPH_TERM_MEMORY_ERROR):fprintf(stderr,"\e[1;31mCould not use OPH_EXEC_MODE [CODE %d]\e[0m\n",OPH_TERM_MEMORY_ERROR);
                                if (print_json) print_oph_term_output_json(hashtbl);
                                if (exec_one_statement) {
                            		oph_term_return = OPH_TERM_MEMORY_ERROR;
                            		break;
                ...
                if (!strstr(cursor,";ncores=") && strncmp(cursor,"ncores=",7)) {
                    if (hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_NCORES)) {
                        if (!flag) {
                            ...
                            char *cursorcopy2 = strdup(cursorcopy);
                            if (!cursorcopy2) {
                                (print_json)?my_fprintf(stderr,"Could not use OPH_NCORES [CODE %d]\\n",OPH_TERM_MEMORY_ERROR):fprintf(stderr,"\e[1;31mCould not use OPH_NCORES [CODE %d]\e[0m\n",OPH_TERM_MEMORY_ERROR);
                                if (print_json) print_oph_term_output_json(hashtbl);
                                if (exec_one_statement) {
                            		oph_term_return = OPH_TERM_MEMORY_ERROR;
                            		break;
                ...
                if (!strstr(cursor,";cube=") && strncmp(cursor,"cube=",5)) {
                    if (hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_DATACUBE)) {
                        if (!flag) {
                            ...
                            char *cursorcopy2 = strdup(cursorcopy);
                            if (!cursorcopy2) {
                                (print_json)?my_fprintf(stderr,"Could not use OPH_DATACUBE [CODE %d]\\n",OPH_TERM_MEMORY_ERROR):fprintf(stderr,"\e[1;31mCould not use OPH_DATACUBE [CODE %d]\e[0m\n",OPH_TERM_MEMORY_ERROR);
                                if (print_json) print_oph_term_output_json(hashtbl);
                                if (exec_one_statement) {
                            		oph_term_return = OPH_TERM_MEMORY_ERROR;
                            		break;
                ...
                if (!strstr(cursor,";cwd=") && strncmp(cursor,"cwd=",4)) {
                    ...
                    if (!flag) {
                    	...
                    	char *cursorcopy2 = strdup(cursorcopy);
                    	if (!cursorcopy2) {
                    		(print_json)?my_fprintf(stderr,"Could not use OPH_CWD [CODE %d]\\n",OPH_TERM_MEMORY_ERROR):fprintf(stderr,"\e[1;31mCould not use OPH_CWD [CODE %d]\e[0m\n",OPH_TERM_MEMORY_ERROR);
                    		if (print_json) print_oph_term_output_json(hashtbl);
                    		if (exec_one_statement) {
                        		oph_term_return = OPH_TERM_MEMORY_ERROR;
                        		break;
        ...
        else if (!strcmp(cursor,OPH_TERM_CMD_VIEW))
        { // VIEW
        	alias_substitutions = 0;
            ...
            if (!cursor) {
            	...
            	if (!strcmp(cursor,"-j")) { //view -j JobID
            		cursor = strtok_r (NULL," \t\n",&saveptr);
            		if (!cursor) {
                		(print_json)?my_fprintf(stderr,"JobID not specified [CODE %d]\\n",OPH_TERM_INVALID_PARAM_VALUE):fprintf(stderr,"\e[1;31mJobID not specified [CODE %d]\e[0m\n",OPH_TERM_INVALID_PARAM_VALUE);
                		if (print_json) print_oph_term_output_json(hashtbl);
                		if (exec_one_statement) {
            	    		oph_term_return = OPH_TERM_INVALID_PARAM_VALUE;
            	    		break;
            		...
            		if (!tmp_cursor) {
                		(print_json)?my_fprintf(stderr,"JobID not valid [CODE %d]\\n",OPH_TERM_INVALID_PARAM_VALUE):fprintf(stderr,"\e[1;31mJobID not valid [CODE %d]\e[0m\n",OPH_TERM_INVALID_PARAM_VALUE);
                		if (print_json) print_oph_term_output_json(hashtbl);
                		if (exec_one_statement) {
            	    		oph_term_return = OPH_TERM_INVALID_PARAM_VALUE;
            	    		break;
            		...
            		if (oph_term_check_wid_mkid(tmp_session, tmp_workflow, tmp_marker, (char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_USER),(char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_PASSWD),(char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SERVER_HOST),(char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SERVER_PORT), &oph_term_return, hashtbl)) {
            			if (print_json) print_oph_term_output_json(hashtbl);
            			if (exec_one_statement)
            	    		break;
            			continue;
            		...
            		if (oph_term_get_request_with_marker(tmp_session,tmp_marker,(char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_USER),(char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_PASSWD),(char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SERVER_HOST),(char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SERVER_PORT),&oph_term_return,&tmp_command,NULL,hashtbl)) {
            			if (print_json) print_oph_term_output_json(hashtbl);
            			if (exec_one_statement)
            	    		break;
            			continue;
					}
            		(print_json)?my_printf("[%s#%s] %s [%s]\\n\\n",tmp_workflow,tmp_marker,(tmp_command)?tmp_command:"",tmp_job_id):printf("\e[1;34m[%s#%s]\e[0m %s \e[1;34m[%s]\e[0m\n\n",tmp_workflow,tmp_marker,(tmp_command)?tmp_command:"",tmp_job_id);
            		if (tmp_command) {
						free(tmp_command);
						tmp_command = NULL;
					}
            		...
            		if (!tmp_cursor) {
#ifndef NO_WORKFLOW
                		(print_json)?my_fprintf(stderr,"Workflow id not valid [CODE %d]\\n",OPH_TERM_INVALID_PARAM_VALUE):fprintf(stderr,"\e[1;31mWorkflow id not valid [CODE %d]\e[0m\n",OPH_TERM_INVALID_PARAM_VALUE);
#else
            		...
            		if (tmp_cursor) { //view wid#mkid
            			...
            			if (oph_term_check_wid_mkid((hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SESSION_ID))?((char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SESSION_ID)):"", tmp_workflow, tmp_marker, (char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_USER),(char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_PASSWD),(char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SERVER_HOST),(char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SERVER_PORT), &oph_term_return, hashtbl)) {
            				if (print_json) print_oph_term_output_json(hashtbl);
            				if (exec_one_statement)
            					break;
            				continue;
            			...
            			char *tmp_command = NULL;
            			if (oph_term_get_request_with_marker((hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SESSION_ID))?((char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SESSION_ID)):"",tmp_marker,(char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_USER),(char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_PASSWD),(char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SERVER_HOST),(char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SERVER_PORT),&oph_term_return,&tmp_command,NULL,hashtbl)) {
            				if (print_json) print_oph_term_output_json(hashtbl);
            				if (exec_one_statement)
            					break;
            				continue;
            			}
            			(print_json)?my_printf("[%s#%s] %s [%s?%s#%s]\\n\\n",tmp_workflow,tmp_marker,(tmp_command)?tmp_command:"",(hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SESSION_ID))?((char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SESSION_ID)):"",tmp_workflow,tmp_marker):printf("\e[1;34m[%s#%s]\e[0m %s \e[1;34m[%s?%s#%s]\e[0m\n\n",tmp_workflow,tmp_marker,(tmp_command)?tmp_command:"",(hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SESSION_ID))?((char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SESSION_ID)):"",tmp_workflow,tmp_marker);
            			if (tmp_command) {
            				free(tmp_command);
            				tmp_command = NULL;
            			}
            			...
            			char *tmp_command = NULL;
            			char *tmp_jobid = NULL;
            			if (oph_term_get_request((hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SESSION_ID))?((char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SESSION_ID)):"",tmp_workflow,(char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_USER),(char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_PASSWD),(char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SERVER_HOST),(char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SERVER_PORT),&oph_term_return,&tmp_command,&tmp_jobid,hashtbl)) {
            				if (print_json) print_oph_term_output_json(hashtbl);
            				if (exec_one_statement)
            					break;
            				continue;
            			}
            			(print_json)?my_printf("[%s] %s [%s]\\n\\n",tmp_workflow,(tmp_command)?tmp_command:"",(tmp_jobid)?tmp_jobid:""):printf("\e[1;34m[%s]\e[0m %s \e[1;34m[%s]\e[0m\n\n",tmp_workflow,(tmp_command)?tmp_command:"",(tmp_jobid)?tmp_jobid:"");
            			if (tmp_command) {
            				free(tmp_command);
            				tmp_command = NULL;
            			}
        ...
        else if (!strcmp(cursor,OPH_TERM_CMD_RESUME))
        { // RESUME
        	alias_substitutions = 0;
		...
		char *tmp_status = NULL;
            	...
            	for (i = last_njobs-1; i >= 0; i--) {
            		...
            		if (oph_term_get_request(tmp_session,buf,(char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_USER),(char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_PASSWD),(char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SERVER_HOST),(char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SERVER_PORT),&oph_term_return,&tmp_command,&tmp_jobid,hashtbl)) {
            			stop = 1;
            			break;
            		}
		...
		if (exit_status) {
            ...
            if ((hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SESSION_ID) && strcmp((char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SESSION_ID),tmp_session)) || !hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SESSION_ID)) {
                if (oph_term_switch_remote_session(tmp_session,(char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_USER),(char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_PASSWD),(char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SERVER_HOST),(char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SERVER_PORT),&oph_term_return,1,hashtbl)) {
                	if (print_json) print_oph_term_output_json(hashtbl);
                	if (exec_one_statement) {
                		oph_term_return = OPH_TERM_GENERIC_ERROR;
                		break;
        ...
        else if (!strcmp(cursor,OPH_TERM_CMD_HELP))
        { // HELP
        	alias_substitutions = 0;
            cursor = strtok_r (NULL," \t\n\"",&saveptr);
            if (!cursor) { // "help"
        ...
        else if (!strcmp(cursor,OPH_TERM_CMD_GETENV))
        { // GETENV
        	alias_substitutions = 0;
            cursor = strtok_r (NULL," \t\n\"",&saveptr);
            if (!cursor) {
        ...
        else if (!strcmp(cursor,OPH_TERM_CMD_ENV))
        { // ENV
        	alias_substitutions = 0;
            cursor = strtok_r (NULL," \t\n\"",&saveptr);
            if (cursor) {
                (print_json)?my_fprintf(stderr,"No arguments required [CODE %d]\\n",OPH_TERM_INVALID_PARAM_VALUE):fprintf(stderr,"\e[1;31mNo arguments required [CODE %d]\e[0m\n",OPH_TERM_INVALID_PARAM_VALUE);
                if (print_json) print_oph_term_output_json(hashtbl);
                if (exec_one_statement) {
            		oph_term_return = OPH_TERM_INVALID_PARAM_VALUE;
            		break;
        ...
        else if (!strcmp(cursor,OPH_TERM_CMD_SETENV))
        { // SETENV
        	alias_substitutions = 0;
            cursor = strtok_r (NULL," \t\n\"=",&saveptr);
            if (!cursor) {
                ...
                if (!key) {
                    (print_json)?my_fprintf(stderr,"Could not extract key [CODE %d]\\n",OPH_TERM_MEMORY_ERROR):fprintf(stderr,"\e[1;31mCould not extract key [CODE %d]\e[0m\n",OPH_TERM_MEMORY_ERROR);
                    if (print_json) print_oph_term_output_json(hashtbl);
                    if (exec_one_statement) {
                		oph_term_return = OPH_TERM_MEMORY_ERROR;
                		break;
        ...
        else if (!strcmp(cursor,OPH_TERM_CMD_UNSETENV))
        { // UNSETENV
        	alias_substitutions = 0;
            cursor = strtok_r (NULL," \t\n\"",&saveptr);
            if (!cursor) {
        ...
        { // UPDATE
        	...
        	if (!cursor) { // "update"
        		...
        		if (hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SERVER_HOST) && hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SERVER_PORT)) {
            		...
            		char *tmpurl = NULL;
            		if (oph_term_env_oph_get_config(OPH_TERM_XML_URL_KEY,hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SERVER_HOST),hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SERVER_PORT),&oph_term_return,&tmpurl,hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_USER),hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_PASSWD),1,hashtbl)) {
            			(print_json)?my_fprintf(stderr,"Unable to get XML folder [CODE %d]\\n",OPH_TERM_GENERIC_ERROR):fprintf(stderr,"\e[1;31mUnable to get XML folder [CODE %d]\e[0m\n",OPH_TERM_GENERIC_ERROR);
            			if (print_json) print_oph_term_output_json(hashtbl);
            			if (exec_one_statement) break;
    					continue;
        			}
        	    	...
        	    	if (mkdir(xml_path,0777)) {
        				if (errno != EEXIST) {
        					(print_json)?my_fprintf(stderr,"Unable to create XML folder %s [CODE %d]\\n",xml_path,OPH_TERM_GENERIC_ERROR):fprintf(stderr,"\e[1;31mUnable to create XML folder %s [CODE %d]\e[0m\n",xml_path,OPH_TERM_GENERIC_ERROR);
        					free(tmpurl);
        					if (print_json) print_oph_term_output_json(hashtbl);
        					if (exec_one_statement) {
        						oph_term_return = OPH_TERM_GENERIC_ERROR;
        			...
        			(print_json)?my_fprintf(stderr,"OPH_SERVER_HOST or OPH_SERVER_PORT not set [CODE %d]\\n",OPH_TERM_INVALID_PARAM_VALUE):fprintf(stderr,"\e[1;31mOPH_SERVER_HOST or OPH_SERVER_PORT not set [CODE %d]\e[0m\n",OPH_TERM_INVALID_PARAM_VALUE);
        			if (print_json) print_oph_term_output_json(hashtbl);
        			if (exec_one_statement) {
        				oph_term_return = OPH_TERM_INVALID_PARAM_VALUE;
        				break;
        		...
        		if (hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SERVER_HOST) && hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SERVER_PORT)) {
        			...
        			if (oph_term_env_rmdir(xml_path_extended)) {
    					(print_json)?my_fprintf(stderr,"Unable to remove XML folder %s [CODE %d]\\n",xml_path_extended,OPH_TERM_GENERIC_ERROR):fprintf(stderr,"\e[1;31mUnable to remove XML folder %s [CODE %d]\e[0m\n",xml_path_extended,OPH_TERM_GENERIC_ERROR);
    					free(tmpurl);
    					if (print_json) print_oph_term_output_json(hashtbl);
    					if (exec_one_statement) {
    						oph_term_return = OPH_TERM_GENERIC_ERROR;
        ...
        else if (!strcmp(cursor,OPH_TERM_CMD_CHECK))
        { // CHECK
        	alias_substitutions = 0;
            ...
            cursor = strtok_r (NULL," \t\n\"",&saveptr);
            if (!cursor) {
                (print_json)?my_fprintf(stderr,"Workflow filename not present [CODE %d]\\n",OPH_TERM_INVALID_PARAM_VALUE):fprintf(stderr,"\e[1;31mWorkflow filename not present [CODE %d]\e[0m\n",OPH_TERM_INVALID_PARAM_VALUE);
                if (print_json) print_oph_term_output_json(hashtbl);
                if (exec_one_statement) {
            		oph_term_return = OPH_TERM_INVALID_PARAM_VALUE;
            		break;
            	...
            	if (!view_metadata) {
            		...
            		if (!hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_GRAPH_LAYOUT) ||
            			...
            			(print_json)?my_fprintf(stderr,"OPH_GRAPH_LAYOUT not set or incorrect [CODE %d]\\n",OPH_TERM_INVALID_PARAM_VALUE):fprintf(stderr,"\e[1;31mOPH_GRAPH_LAYOUT not set or incorrect [CODE %d]\e[0m\n",OPH_TERM_INVALID_PARAM_VALUE);
            			if (print_json) print_oph_term_output_json(hashtbl);
            			if (exec_one_statement) {
            				oph_term_return = OPH_TERM_INVALID_PARAM_VALUE;
            				break;
            	...
            	oph_workflow *tmp_workflow = NULL;
            	if (oph_workflow_load(submission_workflow,"",&tmp_workflow)) {
            		(print_json)?my_fprintf(stderr,"Workflow is NOT a VALID Ophidia Workflow.\\n"):fprintf(stderr,"Workflow is \e[1;31mNOT\e[0m a \e[1;31mVALID\e[0m Ophidia Workflow.\n");
            		if (submission_workflow) {
            			free(submission_workflow);
            			submission_workflow = NULL;
            		}
...
#endif
        else if (!strcmp(cursor,OPH_TERM_CMD_GETALIAS))
        { // GETALIAS
        	alias_substitutions = 0;
        	cursor = strtok_r (NULL," \t\n\"",&saveptr);
        	if (!cursor) {
        ...
        else if (!strcmp(cursor,OPH_TERM_CMD_ALIAS))
        { // ALIAS
        	alias_substitutions = 0;
        	cursor = strtok_r (NULL," \t\n\"",&saveptr);
        	if (cursor) {
        ...
        else if (!strcmp(cursor,OPH_TERM_CMD_SETALIAS))
        { // SETALIAS
        	alias_substitutions = 0;
        	cursor = strtok_r (NULL," \t\n\"=",&saveptr);
        	if (!cursor) {
        		...
        		if (!cursor) {
        			...
        			if (oph_term_setalias(aliases,key,cursor)) {
        				(print_json)?my_fprintf(stderr,"Could not set alias [CODE %d]\\n",OPH_TERM_MEMORY_ERROR):fprintf(stderr,"\e[1;31mCould not set alias [CODE %d]\e[0m\n",OPH_TERM_MEMORY_ERROR);
        				free(key);
        				if (print_json) print_oph_term_output_json(hashtbl);
        				if (exec_one_statement) {
        					oph_term_return = OPH_TERM_MEMORY_ERROR;
        ...
        else if (!strcmp(cursor,OPH_TERM_CMD_UNSETALIAS))
        { // UNSETALIAS
        	alias_substitutions = 0;
        	cursor = strtok_r (NULL," \t\n\"",&saveptr);
        	if (!cursor) {
        ...
        else if (is_alias(aliases,cursor))
        { // alias routine
        	...
        	if (stop) {
        		if (param_string) free(param_string);
        		if (print_json) print_oph_term_output_json(hashtbl);
        		if (exec_one_statement) {
        			oph_term_return = OPH_TERM_GENERIC_ERROR;
        			break;
        ...
        else if (!strcmp(cursor,OPH_TERM_CMD_DEPLOY))
        { // DEPLOY
        	alias_substitutions = 0;

            if (!hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_INFRASTRUCTURE_URL)) {
                (print_json)?my_fprintf(stderr,"OPH_INFRASTRUCTURE_URL not set [CODE %d]\\n",OPH_TERM_INVALID_PARAM_VALUE):fprintf(stderr,"\e[1;31mOPH_INFRASTRUCTURE_URL not set [CODE %d]\e[0m\n",OPH_TERM_INVALID_PARAM_VALUE);
                if (print_json) print_oph_term_output_json(hashtbl);
                if (exec_one_statement) {
            		oph_term_return = OPH_TERM_INVALID_PARAM_VALUE;
            		break;
            ...
            if (!hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_AUTH_HEADER)) {
                (print_json)?my_fprintf(stderr,"OPH_AUTH_HEADER not set [CODE %d]\\n",OPH_TERM_INVALID_PARAM_VALUE):fprintf(stderr,"\e[1;31mOPH_AUTH_HEADER not set [CODE %d]\e[0m\n",OPH_TERM_INVALID_PARAM_VALUE);
                if (print_json) print_oph_term_output_json(hashtbl);
                if (exec_one_statement) {
            		oph_term_return = OPH_TERM_INVALID_PARAM_VALUE;
            		break;
        	...
        	cursor = strtok_r (NULL," \t\n\"",&saveptr);
        	if (!cursor) {
        		(print_json)?my_fprintf(stderr,"RADL filename not present [CODE %d]\\n",OPH_TERM_INVALID_PARAM_VALUE):fprintf(stderr,"\e[1;31mRADL filename not present [CODE %d]\e[0m\n",OPH_TERM_INVALID_PARAM_VALUE);
        		if (print_json) print_oph_term_output_json(hashtbl);
        		if (exec_one_statement) {
        			oph_term_return = OPH_TERM_INVALID_PARAM_VALUE;
        			break;
        		...
        		if (oph_term_env_deploy((const char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_AUTH_HEADER),(const char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_INFRASTRUCTURE_URL),cursor,hashtbl)) {
        			if (print_json) print_oph_term_output_json(hashtbl);
        			if (exec_one_statement) {
        				oph_term_return = OPH_TERM_GENERIC_ERROR;
        				break;
        ...
        else if (!strcmp(cursor,OPH_TERM_CMD_DEPLOY_STATUS))
        { // DEPLOY_STATUS
        	alias_substitutions = 0;
        ...
        else if (!strcmp(cursor,OPH_TERM_CMD_GET_SERVER))
        { // GET_SERVER
        	alias_substitutions = 0;
        	...
        	if (oph_term_env_get_server((const char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_AUTH_HEADER),(const char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_INFRASTRUCTURE_URL),hashtbl)) {
        		if (print_json) print_oph_term_output_json(hashtbl);
        		if (exec_one_statement) {
        			oph_term_return = OPH_TERM_GENERIC_ERROR;
        			break;
        ...
        else if (!strcmp(cursor,OPH_TERM_CMD_UNDEPLOY))
        { // UNDEPLOY
        	alias_substitutions = 0;
        ...
        else if (!strcmp(cursor,OPH_TERM_CMD_DEPLOYS_LIST))
        { // DEPLOYS_LIST
        	alias_substitutions = 0;
        ...
        else if (!strcmp(cursor,OPH_TERM_CMD_DEPLOY_VMS_LIST))
        { // DEPLOY_VMS_LIST
        	alias_substitutions = 0;
