			...
			for (j = 0; j < (size_t)workflow->tasks[i].dependents_indexes_num; j++) {
				...
				for (k = 0; k < workflow->tasks[kk].deps_num; k++) {
					if ((size_t)workflow->tasks[kk].deps[k].task_index == i) {
						if (!workflow->tasks[kk].deps[k].type || !strcmp(workflow->tasks[kk].deps[k].type,"embedded")) snprintf(buffer,OPH_WORKFLOW_RANK_SIZE,"dashed");
						else snprintf(buffer,OPH_WORKFLOW_RANK_SIZE,"solid");
						break;
					}
				}
...
int oph_workflow_print_status(oph_workflow *workflow, int save_img, int open_img, char *json_string_with_status, char *layout) {
	...
	if (save_img) {
	    ...
	    for (i = 0; i < (size_t) workflow->tasks_num; i++) {
	    	...
	    	if (doublecircle) cc += snprintf(dot_string+cc,OPH_WORKFLOW_DOT_MAX_LEN-cc," shape=doublecircle");
	    	else {
	    		if (!strcasecmp(workflow->tasks[i].operator,OPH_OPERATOR_FOR) || !strcasecmp(workflow->tasks[i].operator,OPH_OPERATOR_ENDFOR)) {
	    			cc += snprintf(dot_string+cc,OPH_WORKFLOW_DOT_MAX_LEN-cc," shape=hexagon");
			} else if (!strcasecmp(workflow->tasks[i].operator,OPH_OPERATOR_IF) || !strcasecmp(workflow->tasks[i].operator,OPH_OPERATOR_ELSEIF) || !strcasecmp(workflow->tasks[i].operator,OPH_OPERATOR_ELSE) || !strcasecmp(workflow->tasks[i].operator,OPH_OPERATOR_ENDIF)) {
		...
		for (i = 0; i < (size_t)workflow->tasks_num; i++) {
			for (j = 0; j < (size_t)workflow->tasks[i].dependents_indexes_num; j++) {
				...
				for (k = 0; k < workflow->tasks[kk].deps_num; k++) {
					if ((size_t)workflow->tasks[kk].deps[k].task_index == i) {
						if (!workflow->tasks[kk].deps[k].type || !strcmp(workflow->tasks[kk].deps[k].type,"embedded")) snprintf(color,OPH_WORKFLOW_RANK_SIZE,"dashed");
						else snprintf(color,OPH_WORKFLOW_RANK_SIZE,"solid");
						break;
					}
				}
...
void *main_loop(void *ptr) {
	...
	if (!gtk_init_check( NULL, NULL )) open_img=0;
	...
	if (wf) {
		...
		for (z = 0; z < iterations_num || iterations_num==0; z++) {
			...
			oph_term_client(command_line,tmp_submission_string,NULL,(char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_USER),(char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_PASSWD),(char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SERVER_HOST),(char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SERVER_PORT),oph_term_return,NULL,&response_for_viewer,1,hashtbl);
			...
			if (!oph_term_viewer_check_workflow_status(response_for_viewer) && strcmp((char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_TERM_IMGS),"no_op")) {
				...
				if (oph_term_get_full_request(tmp_session,tmp_workflow,(char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_USER),(char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_PASSWD),(char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SERVER_HOST),(char *)hashtbl_get(hashtbl,OPH_TERM_ENV_OPH_SERVER_PORT),oph_term_return,&full_request,NULL,hashtbl)) {
					(print_json)?my_fprintf(stderr,"Could not get full request [CODE %d]\\n",OPH_TERM_GENERIC_ERROR):fprintf(stderr,"\e[1;31mCould not get full request [CODE %d]\e[0m\n",OPH_TERM_GENERIC_ERROR);
					if (response_for_viewer) free(response_for_viewer);
...
int oph_workflow_get_ranks_string(oph_workflow_task *tasks, int tasks_num, char **ranks_string) {
	...
	*ranks_string = (char *)calloc(tasks_num*OPH_WORKFLOW_RANK_SIZE,sizeof(char));
	...
	while (prev_tasks_num) {
		k=0;
		for (i = 0; i < prev_tasks_num; i++) cur_tasks_num += tasks[prev_tasks[i]].dependents_indexes_num;
		if (!cur_tasks_num) {
			if (prev_tasks) {
				free(prev_tasks);
				prev_tasks = NULL;
...
int workflow_is_child_of(oph_workflow *wf, int p, int c)
{
	...
	for (i=0;i<wf->tasks[p].dependents_indexes_num;++i)
	{
		...
		if (workflow_is_child_of(wf, j, c)) return 1;
	}
	return 0;
}
