...
int oph_viewer_get_ranks_string(oph_json_links *nodelinks, unsigned int nodelinks_num, char **ranks_string) {
	...
	*ranks_string = (char *)calloc(nodelinks_num,1000);
	...
	while (prev_nodes_num) {
		k=0;
		for (i = 0; i < prev_nodes_num; i++) cur_nodes_num += nodelinks[prev_nodes[i]].links_num;
		if (!cur_nodes_num) {
			if (prev_nodes) {
				free(prev_nodes);
				prev_nodes = NULL;
...
void adjust_string_array(char ***string_array, char **input_strings, int input_strings_num) {
	if (!string_array || !input_strings || input_strings_num<1) return;
	size_t i;
	for (i = 0; i < (size_t)input_strings_num; i++) if (!input_strings[i]) return;
...
int adjusted_string_max_len(char *not_adjusted_string) {
	if (!not_adjusted_string) return 0;
	int max_len = 0;
	int cur_len = 0;
	size_t i;
...
void print_grid(oph_json_obj_grid *obj, const char *color_string, int show_list) {
	...
	if (!max_lengths) return;
	for (j = 0; j < obj->keys_num; j++) max_lengths[j] = adjusted_string_max_len(obj->keys[j]);
    ...
    int total_space = 0;
    for (j = 0; j < obj->keys_num; j++) total_space += max_lengths[j];
	...
	if (!cursors) {
		...
		free(real_lengths);
		size_t k;
	    for (k = 0; k < obj->keys_num; k++) free(adjusted_header[k]);
    ...
    int newline_found = 0;
	char single_char[2];
	for (i = 0; i < obj->values_num2; i++) if (strlen(adjusted_header[i])==0 || real_lengths[i]==0) completed++;
    		...
    		for (j = 0; j < (size_t)real_lengths[i]; j++) {
				if ((strlen(adjusted_header[i]) - cursors[i]) > 0) {
					if (adjusted_header[i][cursors[i]] == '\n') {
						...
						cursors[i]++;
						if (cursors[i] == (int)strlen(adjusted_header[i])) completed++;
					}
				} else {
					printf(" ");
				}
    ...
    for (j = 0; j < obj->keys_num; j++) free(adjusted_header[j]);
    free(adjusted_header);
    free(cursors);
    ...
    for (r = 0; r < obj->values_num1; r++) {
    	...
    	if (!cursors) {
    		...
    		free(real_lengths);
    		size_t k;
    	    for (k = 0; k < obj->values_num2; k++) free(adjusted_row[k]);
    	    free(adjusted_row);
    		return;
    	}
        int completed = 0;
        int newline_found = 0;
        for (i = 0; i < obj->values_num2; i++) if (strlen(adjusted_row[i])==0 || real_lengths[i]==0) completed++;
        		...
        		for (j = 0; j < (size_t)real_lengths[i]; j++) {
    				if ((strlen(adjusted_row[i]) - cursors[i]) > 0) {
    					if (adjusted_row[i][cursors[i]] == '\n') {
    						...
    						cursors[i]++;
    						if (cursors[i] == (int)strlen(adjusted_row[i])) completed++;
    					}
    				} else {
    					printf(" ");
    				}
        ...
        //row cleanup
        for (i = 0; i < obj->values_num2; i++) free(adjusted_row[i]);
        free(adjusted_row);
        free(cursors);
	}
...
void print_multigrid(oph_json_obj_multigrid *obj, const char *color_string) {
	...
	if (!max_lengths_rowkeys) return;
	for (j = 0; j < obj->rowkeys_num; j++) max_lengths_rowkeys[j] = adjusted_string_max_len(obj->rowkeys[j]);
	...
	for (j = 0; j < obj->rowkeys_num; j++) imp_dim_header_length += max_lengths_rowkeys[j];
	...
	if (!max_lengths_values) {
		if (max_lengths_rowkeys) free(max_lengths_rowkeys);
		return;
	}
	...
	for (j = 0; j < obj->measurevalues_num2; j++) measure_header_length += max_lengths_values[j];
    ...
    available_space = (available_space < 0)?0:available_space;
    int total_space = 0;
    for (j = 0; j < obj->rowkeys_num; j++) total_space += max_lengths_rowkeys[j];
    for (j = 0; j < obj->measurevalues_num2; j++) total_space += max_lengths_values[j];
	...
	for (j = 0; j < obj->rowkeys_num; j++) real_imp_dim_header_length += real_lengths_rowkeys[j];
	// compute real length for measure header
	int real_measure_header_length = (obj->measurevalues_num2 - 1)*3;
	for (j = 0; j < obj->measurevalues_num2; j++) real_measure_header_length += real_lengths_values[j];
	...
	for (i = 0; i < obj->colkeys_num; i++) {
		...
		for (j = 0; j < obj->measurevalues_num2; j++) tmprow[j+1] = obj->colvalues[j][i];
    	...
    	if (!cursors) {
			...
			free(tmprow);
			size_t k;
    	    for (k = 0; k < obj->measurevalues_num2+1; k++) free(adjusted_row[k]);
    	    free(adjusted_row);
    		return;
    	}
        int completed = 0;
        int newline_found = 0;
        char single_char[2];
        for (j = 0; j < obj->measurevalues_num2; j++) if (strlen(adjusted_row[j+1])==0 || real_lengths_values[j+1]==0) completed++;
        if (strlen(adjusted_row[0])==0 || real_imp_dim_header_length==0) completed++;
    		...
    		for (j = 0; j < (size_t)real_imp_dim_header_length; j++) {
				if ((strlen(adjusted_row[0]) - cursors[0]) > 0) {
					if (adjusted_row[0][cursors[0]] == '\n') {
						...
						printf(color_string,single_char);
						cursors[0]++;
						if (cursors[0] == (int)strlen(adjusted_row[0])) completed++;
					}
				} else {
					printf(" ");
				}
        		...
        		for (j = 0; j < (size_t)real_lengths_values[k-1]; j++) {
    				if ((strlen(adjusted_row[k]) - cursors[k]) > 0) {
    					if (adjusted_row[k][cursors[k]] == '\n') {
    						...
    						cursors[k]++;
    						if (cursors[k] == (int)strlen(adjusted_row[k])) completed++;
    					}
    				} else {
    					printf(" ");
    				}
        ...
        //row cleanup
		free(tmprow);
	    for (j = 0; j < obj->measurevalues_num2+1; j++) free(adjusted_row[j]);
	    free(adjusted_row);
	    free(cursors);
	}
	...
	for (j = 0; j < obj->rowkeys_num; j++) tmprow[j] = obj->rowkeys[j];
	...
	if (!cursors) {
		...
		free(tmprow);
		size_t k;
	    for (k = 0; k < obj->rowkeys_num+1; k++) free(adjusted_row[k]);
	    free(adjusted_row);
		return;
	}
    int completed = 0;
    int newline_found = 0;
    char single_char[2];
    for (j = 0; j < obj->rowkeys_num; j++) if (strlen(adjusted_row[j])==0 || real_lengths_rowkeys[j]==0) completed++;
    if (strlen(adjusted_row[obj->rowkeys_num])==0 || real_measure_header_length==0) completed++;
		...
		for (j = 0; j < (size_t)real_measure_header_length; j++) {
			if ((strlen(adjusted_row[obj->rowkeys_num]) - cursors[obj->rowkeys_num]) > 0) {
				if (adjusted_row[obj->rowkeys_num][cursors[obj->rowkeys_num]] == '\n') {
					...
					if (cursors[obj->rowkeys_num] == (int)strlen(adjusted_row[obj->rowkeys_num])) completed++;
				}
			} else {
				printf(" ");
			}
    ...
    // cleanup
	free(tmprow);
    for (j = 0; j < obj->rowkeys_num+1; j++) free(adjusted_row[j]);
    free(adjusted_row);
    free(cursors);
	...
	for (i = 0; i < obj->measurevalues_num1; i++) {
		...
		for (j = 0; j < obj->rowkeys_num; j++) tmprow[j] = obj->rowvalues[i][j];
		for (j = obj->rowkeys_num; j < obj->rowkeys_num + obj->measurevalues_num2; j++) tmprow[j] = obj->measurevalues[i][j];
    	...
    	if (!cursors) {
			...
			free(tmprow);
			size_t k;
    	    for (k = 0; k < obj->rowkeys_num + obj->measurevalues_num2; k++) free(adjusted_row[k]);
    	    free(adjusted_row);
    		return;
    	}
        int completed = 0;
        int newline_found = 0;
        for (j = 0; j < obj->rowkeys_num; j++) if (strlen(adjusted_row[j])==0 || real_lengths_rowkeys[j]==0) completed++;
        for (j = obj->rowkeys_num; j < obj->rowkeys_num + obj->measurevalues_num2; j++) if (strlen(adjusted_row[j])==0 || real_lengths_values[j]==0) completed++;
        ...
        //row cleanup
		free(tmprow);
	    for (j = 0; j < obj->rowkeys_num + obj->measurevalues_num2; j++) free(adjusted_row[j]);
	    free(adjusted_row);
	    free(cursors);
	}
...
void print_tree(oph_json_obj_tree *obj, const char *color_string, int save_img, int open_img, char *session_code, char *markerid, char *layout) {
    	...
    	for (j = 0; j < obj->nodevalues_num2; j++) {
    		if (obj->nodekeys[j] && obj->nodevalues[i][j]) {
    			if (strcmp(obj->nodekeys[j],"") && strcmp(obj->nodekeys[j],"-") && strcmp(obj->nodevalues[i][j],"") && strcmp(obj->nodevalues[i][j],"-")) {
    				cc += snprintf(dot_string+cc,OPH_TERM_VIEWER_DOT_MAX_LEN-cc,"%s : %s\\n",(obj->nodekeys[j])?(obj->nodekeys[j]):"-",(obj->nodevalues[i][j])?(obj->nodevalues[i][j]):"-");
    				ccc += snprintf(printable_dot_string+ccc,OPH_TERM_VIEWER_DOT_MAX_LEN-ccc,"%s : %s\\n",(obj->nodekeys[j])?(obj->nodekeys[j]):"-",(obj->nodevalues[i][j])?(obj->nodevalues[i][j]):"-");
				}
    		}
		}
	...
	for (i = 0; i < obj->nodelinks_num; i++) {
		for (j = 0; j < obj->nodelinks[i].links_num; j++) {
			cc += snprintf(dot_string+cc,OPH_TERM_VIEWER_DOT_MAX_LEN-cc,"%d->%s [label=\"%s\"] ",(int)i,(obj->nodelinks[i].links[j].node)?(obj->nodelinks[i].links[j].node):"0",(obj->nodelinks[i].links[j].description)?(obj->nodelinks[i].links[j].description):"");
			ccc += snprintf(printable_dot_string+ccc,OPH_TERM_VIEWER_DOT_MAX_LEN-ccc,"\t%d->%s\t[label=\"%s\"]\n",(int)i,(obj->nodelinks[i].links[j].node)?(obj->nodelinks[i].links[j].node):"0",(obj->nodelinks[i].links[j].description)?(obj->nodelinks[i].links[j].description):"");
		}
	}
...
int oph_term_viewer_dump(char **json_string, char **newdatacube, char **newcwd) {
	...
	if (print_json)
	{
		if (oph_term_response) free(oph_term_response);
...
int oph_term_viewer_basic(char **json_string, const char *color, int save_img, int open_img, int show_list, char **newdatacube, char **newcwd, char *layout) {
	...
	for (i = 0; i < json->response_num; i++) {
		...
		if (!strcmp(response_i->objkey,"status") && json->response_num > 1) continue; // status already processed
	...
	if (json->response_num >= 1) {
		...
		for (i = 0; i < json->response_num; i++) {
			if (!strcmp(json->response[i].objkey,"resume")) {
				if (json->response[i].objcontent_num >= 1) {
					for (j = 0; j < (((oph_json_obj_grid *) json->response[i].objcontent)[0]).values_num1; j++) {
						if (!strcmp(wid,(((oph_json_obj_grid *) json->response[i].objcontent)[0]).values[j][2]) && !strcmp(mkid,(((oph_json_obj_grid *) json->response[i].objcontent)[0]).values[j][3])) {
							found = 1;
							break;
						}
					}
	...
	if (json->response_num >= 1) {
		...
		for (i = 0; i < json->response_num; i++) {
			if (!strcmp(json->response[i].objkey,"workflow_status")) {
				if (!strcmp(((oph_json_obj_text *) json->response[i].objcontent)[0].message,"OPH_STATUS_COMPLETED") || strstr(((oph_json_obj_text *) json->response[i].objcontent)[0].message,"ERROR")) {
					if (json) oph_json_free(json);
					return OPH_TERM_SUCCESS;
				} else {
					if (json) oph_json_free(json);
