...
void print_cmd_list() {
    ...
    for (i = 0; i < env_vars_num; i++) {
        if (!strcmp(env_vars[i],OPH_TERM_ENV_OPH_NCORES)) {
            ...
            (print_json)?my_printf("\\t%25s = %s",env_vars[i],OPH_TERM_HELP_OPH_WORKFLOW_AUTOVIEW_SHORT):printf("\t%25s = %s",env_vars[i],OPH_TERM_HELP_OPH_WORKFLOW_AUTOVIEW_SHORT);
        }
#ifdef WITH_IM_SUPPORT
        else if (!strcmp(env_vars[i],OPH_TERM_ENV_OPH_INFRASTRUCTURE_URL)) {
        	(print_json)?my_printf("\\t%25s = %s",env_vars[i],OPH_TERM_HELP_OPH_INFRASTRUCTURE_URL_SHORT):printf("\t%25s = %s",env_vars[i],OPH_TERM_HELP_OPH_INFRASTRUCTURE_URL_SHORT);
        }
